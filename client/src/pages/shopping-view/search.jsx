...
function SearchProducts() {
  ...
  useEffect(() => {
    ...
    if (keywordFromParams) {
      console.log("Search keyword from params:", keywordFromParams);
      setKeyword(keywordFromParams);
      dispatch(getSearchResults(keywordFromParams));
    } else {
      console.log("No keyword found, resetting search results");
  ...
  useEffect(() => {
    if (keyword.trim().length > 0) {
      console.log("Setting search params with keyword:", keyword);
      const timer = setTimeout(() => {
        setSearchParams({ keyword });
      }, 1000);
      return () => clearTimeout(timer);
    } else {
      console.log("Keyword is too short, resetting search results");
      dispatch(resetSearchResults());
    }
